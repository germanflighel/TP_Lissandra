...
int validarParametros(int header, char* parametros) {
	...
	int fill_package_create(t_PackageCreate* package, char* parametros) {
		...
		package->total_size = sizeof(package->header)
				+ sizeof(package->tabla_long) + sizeof(package->consistency)
				+ sizeof(package->compaction_time) + sizeof(package->partitions)
				+ package->tabla_long;
	...
	int recieve_and_deserialize_describe_request(t_PackageDescribe *package,
		...
		status = recv(socketCliente, package->nombre_tabla, package->tabla_long,
				0);
		if (!status)
			return 0;
	...
	char* serializarSelect(t_PackageSelect *package) {
		...
		size_to_send = sizeof(package->tabla_long);
		memcpy(serializedPackage + offset, &(package->tabla_long),
				size_to_send);
		offset += size_to_send;
	...
	char* serializarCreate(t_PackageCreate *package) {
		...
		memcpy(serializedPackage + offset, &(package->consistency),
				size_to_send);
		offset += size_to_send;
		...
		memcpy(serializedPackage + offset, &(package->partitions),
				size_to_send);
	...
	int fill_package_insert(t_PackageInsert *package, char* parametros,
			int filesys) {
		...
		if (fs != 2) {
			...
			memcpy(package->tabla, parametrosSeparados[0],
					package->tabla_long + 1);

			package->value_long = strlen(parametrosSeparados[2]) - 2;

			package->value = malloc(package->value_long + 1);
			memcpy(package->value, parametrosSeparados[2] + 1,
					package->value_long);
		...
		package->total_size = sizeof(package->header)
				+ sizeof(package->value_long) + sizeof(package->tabla_long)
				+ sizeof(package->key) + sizeof(package->timestamp)
				+ package->value_long + package->tabla_long;
	...
	char* serializarInsert(t_PackageInsert *package) {
		...
		memcpy(serializedPackage + offset, &(package->value_long),
				size_to_send);
		offset += size_to_send;
	...
	int recieve_and_deserialize_select(t_PackageSelect *package,
			int socketCliente) {
...
//	printf("Key: %d \n", package->key);
	...
	int recieve_and_deserialize_create(t_PackageCreate *package,
			int socketCliente) {
		...
		status = recv(socketCliente, buffer, sizeof(package->compaction_time),
				0);
	...
	int recieve_and_deserialize_insert(t_PackageInsert *package,
			int socketCliente) {
	...
	void enviar_describe(char* tabla, int socket) {
		...
		send(socket, serializedPackage,
				sizeof(int) + sizeof(int) + strlen(tabla), 0);
	...
	int recieve_and_deserialize_handshake(t_Handshake *package,
			int socketCliente) {
